...
<div align="center">
  <a href="https://github.com/ojbkxc/RustSync">
    <img height="64" alt="RustSync" src="./rustsync-logo.svg"/>
  </a>
  <p><em>RustSync is an automated synchronization tool with a built-in storage engine and OpenList/AList v3+ compatibility.</em></p>
  <div>
    <a href="https://github.com/ojbkxc/RustSync/blob/main/LICENSE">
      <img src="https://img.shields.io/github/license/ojbkxc/RustSync" alt="License" />
    </a>
    ...
    <a href="https://github.com/ojbkxc/RustSync/releases">
      <img src="https://img.shields.io/github/release/ojbkxc/RustSync" alt="Latest version" />
    </a>
    <a href="https://github.com/ojbkxc/RustSync/releases">
...
RustSync is updated frequently, so these screenshots are for reference only. Refer to the current interface for the latest design.
...
> [!IMPORTANT]
> RustSync includes a built-in, non-removable `RustSync` engine, so OpenList is no longer required. You can still add an external OpenList/AList instance when you need its additional drivers.

> [!WARNING]
> **Do not expose RustSync directly to the public internet. You do so at your own risk!**
>
> RustSync includes some security measures, but absolute security cannot be guaranteed. If external access is necessary, use a strong password and enable `SSL`.
...
RustSync can scan specified directories for differences on a schedule. Full sync mode makes the destination match the source, while Add only mode copies only files that exist in the source but not in the destination.

#### 2. Scheduled Downloads

You can create a one-time job by specifying the year, month, day, hour, minute, and second using `cron`. This lets RustSync automatically download files from a specific cloud drive to local storage during off-peak hours.
...
* Clean removal: simply delete RustSync when you no longer need it; it leaves no residual files or dependencies and does not affect other applications
...
* Runs offline except for storage services configured by the user and never uploads user data to the RustSync project
...
* A built-in, non-removable `RustSync` engine with local directory, SMB, FTP/FTPS, SFTP (SSH), and Aliyun Drive Open Platform support
...
* Controllable storage usage: configure retention periods for task records and logs to keep RustSync's storage footprint within a predictable range
* Notifications through DingTalk group bots or ServerChan after a task succeeds or fails

## Usage

### Built-in RustSync Engine

The `RustSync` engine is created automatically on first startup and initially has no directories. Open its directory manager and add a virtual directory backed by one of these storage types:

* Local directory: browse and select an existing absolute path visible to the RustSync process. For Docker, mount the host directory into the container first.
* SMB: an SMB 2/3 server and share, credentials, and an optional root within the share. The host field can scan the `/24` networks of RustSync's active private IPv4 addresses for devices accepting TCP 445; manual entry remains available.
* FTP: FTP or explicit TLS (AUTH TLS), credentials, and an optional remote root. The server must support `MLSD`, which RustSync uses to distinguish files, directories, and unsafe link-like entries without leaving the configured root.
...
* Aliyun Drive: create an application in the [Aliyun Drive Developer Portal](https://www.aliyundrive.com/developer), then obtain `client_id`, `client_secret`, and `refresh_token` through official OAuth. RustSync uses the official OpenFile API at `openapi.alipan.com` and persists rotated refresh tokens automatically.

After selecting `RustSync` in a job, paths appear as `/<virtual-directory-name>/...`. File-size comparison, copying, directory creation, deletion, and progress tracking run inside RustSync. Copies between different storage backends stream through the RustSync process.

SFTP v3 cannot provide a portable no-follow guarantee across every server-side path race. Use an SSH account restricted by server-side `chroot`, or make the configured remote root writable only by that account. Storage credentials, including SFTP passwords and private keys, currently follow the existing engine behavior and are stored in RustSync's SQLite database without at-rest encryption; protect the `data` directory accordingly.
...
Download the executable for your platform from [Releases](https://github.com/ojbkxc/RustSync/releases) and run it directly.
...
Use the following Compose file to start RustSync and OpenList together:
...
services:
  ...
  rust-sync:
    ...
    ports:
      - "8023:8023"
    volumes:
      - ./rustsync-data:/app/data
```

After startup, open OpenList at `http://127.0.0.1:5244` and RustSync at `http://127.0.0.1:8023`. When adding the engine in RustSync, use `http://openlist:5244` as the OpenList URL. The service name works as the hostname inside the Compose network.
...
After signing in, open Engine Management and add directories to the built-in RustSync engine, or add an external OpenList/AList engine. Then go to Job Management and create a synchronization job.
...
Configuration precedence: `data/config.ini` > `environment variables` > `default values`. When a higher-priority source is present, all lower-priority sources are **ignored**. Restart RustSync or its Docker container after changing the configuration.
...
This file does not exist by default. To use it, create `config.ini` in the `data` directory next to the RustSync executable and add the content shown above. The file must use `UTF-8` encoding.

| config.ini    | Docker environment variable | Description | Default |
| ------------- | --------------------------- | ----------- | ------- |
| password      | RUSTSYNC_PASSWORD                | Initial administrator password; only used when the database is first created. `RANDOM` or an empty value generates a random password. The legacy `RUSTSYNC_PASSWD` variable is also supported | RANDOM |
| port          | RUSTSYNC_PORT                    | Listening port | 8023 |
| expires       | RUSTSYNC_EXPIRES                 | Login session lifetime, in days | 2 |
| log_level     | RUSTSYNC_LOG_LEVEL               | Log level: 0-DEBUG, 1-INFO, 2-WARNING, 3-ERROR, 4-CRITICAL. Higher values produce fewer logs; 1 or 2 is recommended | 1 |
| console_level | RUSTSYNC_CONSOLE_LEVEL           | Console log level; available in v0.2.3 and later. Uses the same levels as above | 2 |
| log_save      | RUSTSYNC_LOG_SAVE                | System log retention period, in days. Older logs are removed automatically; 0 disables automatic cleanup | 7 |
| task_save     | RUSTSYNC_TASK_SAVE               | Task record retention period, in days. Older records are removed automatically; 0 disables automatic cleanup | 0 |
| task_timeout  | RUSTSYNC_TASK_TIMEOUT            | Task execution timeout, in hours. Set this high enough for large backups to complete | 72 |
...
* Improve the Windows version, including launch at startup, hidden-window operation, start/stop controls, and more [#13](https://github.com/ojbkxc/RustSync/issues/13)
* Support encrypted synchronization through OpenList [#18](https://github.com/ojbkxc/RustSync/issues/18)
...
* One-command scripts to install, update, and uninstall RustSync on Linux
