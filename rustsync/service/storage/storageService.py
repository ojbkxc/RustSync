...
def _requireRustSync(engineId):
    engine = getEngineById(int(engineId))
    if engine.get("engineType") != "rustsync" or engine.get("systemKey") != "rustsync":
        raise ValueError("storage directories can only be managed for the RustSync engine")
    return engine
...
def enginePathsOverlap(engineId, firstPath, secondPath):
    engine = getEngineById(int(engineId))
    isRustSync = (
        engine.get("engineType") == "rustsync" and engine.get("systemKey") == "rustsync"
    )
    if not isRustSync:
