...
class _CopyTask:
    ...
    def start(self):
        self.thread = threading.Thread(target=self._run, name="rustsync-copy-" + self.id[:8], daemon=True)
        self.thread.start()
...
class RustSyncClient:
    """AList-compatible facade over RustSync's internal storage drivers."""
