...
from service.engine.rustsyncClient import RustSyncClient
...
def getClientById(engineId):
    ...
    # this engine, so one offline AList cannot block RustSync or other engines.
    with _getEngineLock(engineId):
        ...
        if engine.get("engineType") == "rustsync":
            client = RustSyncClient(engineId)
        else:
