...
const engineOptionLabel = (engine) => {
  if (engine.engineType === "rustsync") return engine.displayName || "RustSync";
  const name = engine.url || engine.displayName || engine.userName || t("engine.alist");
  return `${name}${engine.remark ? ` [${engine.remark}]` : ""}`;
};

const engineOptionMeta = (engine) => (engine.engineType === "rustsync" ? t("engine.internal") : engine.userName);
