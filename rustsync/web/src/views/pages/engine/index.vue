...
const isRustSync = (engine) => engine?.engineType === "rustsync";

const engineTitle = (engine) => {
  if (isRustSync(engine)) return engine.displayName || "RustSync";
...
<template>
  <div class="engine">
    ...
    <div v-else class="card-box">
      <div v-for="item in alistList" :key="item.id" class="card-item" :class="{ 'rustsync-card': isRustSync(item) }">
        <div class="card-item-top">
          <el-image :src="isRustSync(item) ? '/logo.png' : '/alist.svg'" fit="contain" class="engine-logo" />
          <div class="engine-info">
            <div class="card-item-user">
              <span>{{ engineTitle(item) }}</span>
              <span v-if="!isRustSync(item) && item.remark" class="card-item-remark">[{{ item.remark }}]</span>
            </div>
            <div v-if="isRustSync(item)" class="engine-meta">
    ...
    <el-dialog
      ...
      :title="$t('engine.directoryManagementTitle', { name: currentEngine?.displayName || 'RustSync' })"
      :append-to-body="true"
      :close-on-click-modal="false"
    >
