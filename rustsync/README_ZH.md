...
<div align="center">
  ...
  <p><em>RustSync 是一个带内置存储引擎、并兼容 OpenList/AList v3+ 的自动化同步工具。</em></p>
  <div>
...
> [!IMPORTANT]
> RustSync 默认提供不可删除的内置 `RustSync` 引擎，无需额外部署 OpenList。需要使用更多 OpenList/AList 驱动时，仍可把外部实例添加为引擎。
...
* 除用户配置的存储服务外完全离线运行，不向 RustSync 项目方上传用户数据
* 完善的错误处理，稳定可靠，逻辑自洽；可能出错，但永不崩溃（我猜的）
* 完善的日志，所有错误都会被记录
* 内置不可删除的 `RustSync` 引擎，支持本地目录、SMB、FTP/FTPS、SFTP（SSH）和阿里云盘开放平台
...
首次启动后，引擎管理中会自动出现 `RustSync`，其初始目录为空。点击目录管理，为每个虚拟目录选择一种存储：

* 本地目录：可直接浏览并选择 RustSync 进程可访问的现有绝对目录；Docker 中需要先把宿主机目录挂载进容器。
* SMB：填写服务器、共享名、账号以及可选的共享内根目录，使用 SMB 2/3。主机字段可以扫描 RustSync 活动私网 IPv4 地址所在的 `/24` 网段，列出 TCP 445 可连接设备；也可以继续手动填写。
* FTP：支持 FTP 与显式 TLS（AUTH TLS），填写服务器、账号以及可选的远端根目录。服务端必须支持 `MLSD`，RustSync 依靠它区分文件、目录和不安全的链接类条目，确保操作不会越出配置的根目录。
...
* 阿里云盘：在[阿里云盘开发者门户](https://www.aliyundrive.com/developer)创建应用，通过官方 OAuth 获取 `client_id`、`client_secret` 和 `refresh_token`。RustSync 调用 `openapi.alipan.com` 的官方 OpenFile API，并自动保存轮换后的 refresh token。

作业中选择 `RustSync` 后，路径以 `/<虚拟目录名>/...` 显示。复制、建目录、删除、文件大小比较和任务进度都由进程内部完成；不同存储之间的复制会由 RustSync 中转文件内容。

SFTP v3 无法在所有服务端路径竞态下提供通用的“禁止跟随链接”保证，建议使用服务端 `chroot` 限制 SSH 账号，或确保配置的远端根目录仅该账号可写。存储凭据（包括 SFTP 密码和私钥）目前沿用现有引擎行为，未做静态加密而直接保存在 RustSync 的 SQLite 数据库中，请妥善保护 `data` 目录。
...
前往[Release](https://github.com/ojbkxc/RustSync/releases)下载对应平台的可执行程序，直接执行
...
使用以下 Compose 配置可以同时启动 RustSync 和 OpenList：
...
启动后，可通过 `http://127.0.0.1:5244` 访问 OpenList，通过 `http://127.0.0.1:8023` 访问 RustSync。在 RustSync 中添加引擎时，OpenList 地址应填写 `http://openlist:5244`；Compose 网络内可以直接使用服务名作为主机名。
...
进入系统后先到`引擎管理`为内置 RustSync 引擎添加目录，或添加外部 OpenList/AList 引擎，然后前往`作业管理`创建同步作业
...
| ------------- | ----------------- | ------------------------------------------------------------ |---------------|
| password      | RUSTSYNC_PASSWORD      | 初始管理员密码，仅首次创建数据库时生效；`RANDOM`或空值表示随机生成；兼容旧变量`RUSTSYNC_PASSWD` | RANDOM        |
| port          | RUSTSYNC_PORT          | 运行端口号                                                   | 8023          |
| expires       | RUSTSYNC_EXPIRES       | 登录有效期，单位天                                           | 2             |
| log_level     | RUSTSYNC_LOG_LEVEL     | 日志等级：0-DEBUG，1-INFO，2-WARNING，3-ERROR，4-CRITICAL；数值越大，产生的日志越少，推荐1或2 | 1             |
| console_level | RUSTSYNC_CONSOLE_LEVEL | 控制台日志等级：适用于v0.2.3及之后版本；与上同               | 2             |
| log_save      | RUSTSYNC_LOG_SAVE      | 系统日志保留天数，该天数之前的日志会自动清理，单位天，0表示不自动清理 | 7             |
| task_save     | RUSTSYNC_TASK_SAVE     | 任务记录保留天数，该天数之前的记录会自动清理，单位天，0表示不自动清理 | 0             |
| task_timeout  | RUSTSYNC_TASK_TIMEOUT  | 任务执行超时时间，单位小时。一定要设置长一点，以免要备份的东西太多 | 72            |
| -             | TZ                | 时区                                                         | Asia/Shanghai |
...
* windows版本优化（开机自启，隐藏页面，启动停止等）[#13](https://github.com/ojbkxc/RustSync/issues/13)
* OpenList支持加密同步 [#18](https://github.com/ojbkxc/RustSync/issues/18)
* 本地引擎支持加密同步
* 保留历史N个版本（N可自定义，可无限）
* 配置导入导出
* linux一键安装、更新与卸载脚本
